...
- **Approve** a spec by merging its pull request, titled
  ...
  **For the owner** card. To ask for a change instead, comment on the pull
  request.
- **The queue** builds one bead at a time, in the order specs were merged,
  once every bead on a spec's `Depends on:` line is implemented. It pauses
  while any pull request is labelled `needs-human`.
