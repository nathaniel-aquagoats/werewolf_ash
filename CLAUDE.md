...
- Background work goes through Oban (owner decision 2026-09-13): work an action triggers that need not happen in the same instant (a follow-up check such as the win check after a wolf kill, an email, a notification) is enqueued as an Oban job, through an AshOban trigger or an Oban worker, inside the action's own transaction. The enqueue is synchronous; the job only runs after the action commits, so it reads committed state. Never spawn a Task, an async Reactor or any other process from inside an action to do such work. Work that must be atomic with the action stays inside the transaction and synchronous: rule validations, the phase change itself, and the lynch and win check at dusk, which run inside `end_day`. Jobs must be safe to run twice: lock what they change, re-check state first, and use Oban uniqueness per record. Test with Oban's `testing: :manual`: assert the job was enqueued, then perform it
