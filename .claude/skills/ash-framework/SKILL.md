---
name: ash-framework
description: "Use when working with Ash Framework or any ash_* extension: domains, resources, actions, policies, AshPostgres, AshOban, AshGraphql, AshAuthentication. Consult before any domain change."
metadata:
...
```sh
mix usage_rules.search_docs "search term" -p ash -p ash_authentication -p ash_graphql -p ash_oban -p ash_postgres -p ash_state_machine
```
